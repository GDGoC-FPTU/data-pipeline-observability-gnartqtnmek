...
def extract(file_path):
    ...
    try:
        ...
        print("Extract warning: JSON content is not a list. Returning empty list.")
        return []
...
def transform(data):
    ...
    # Ensure expected columns exist even when there are no valid records.
...
def load(df, output_path):
    ...
    """
    df.to_csv(output_path, index=False)
    print(f"Load complete: {len(df)} records loaded to {output_path}")
